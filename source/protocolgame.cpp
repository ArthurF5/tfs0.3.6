	...
	OperatingSystem_t operatingSystem, uint16_t version, bool gamemaster, bool castAccount)
{
	...
	if((!_player || name == "Account Manager" || g_config.getNumber(ConfigManager::ALLOW_CLONES) > (int32_t)players.size()) && !castAccount)
	{
		...
		player->setID();
#ifdef __ANTIMAGEBOMB__
        PlayerVector temp;
        temp = g_game.getPlayersByIP(player->getIP());
        if((int32_t)temp.size() >= 20)
        {
                disconnectClient(0x14, "Only 20 connections allowed from the same IP.");
                return false;
        }
#endif
...
void ProtocolGame::parsePlayerPurchase(NetworkMessage &msg)
{
    ...
    bool ignoreCap = false; // never ignore cap
    bool inBackpacks = false; // never buy with backpack
	...
	if(creature)
	{
		...
		switch(type)
		{
			...
			case SPEAK_RVR_ANSWER:
				...
				break;
			default:
				msg->AddString(!creature->getHideName() ? pname: ""); //CA
				break;
		}
