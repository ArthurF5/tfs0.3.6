...
bool ConfigManager::load()
{
	...
	if(!m_loaded) //info that must be loaded one time (unless we reset the modules involved)
	{
		...
		m_confString[SQL_DB] = getGlobalString("sqlDatabase", "thelostserver");
		m_confString[SQL_USER] = getGlobalString("sqlUser", "root");
		m_confString[SQL_PASS] = getGlobalString("sqlPass", "");
		m_confString[SQL_FILE] = getGlobalString("sqlFile", "lostserver.s3db");
		...
		m_confString[MAP_NAME] = getGlobalString("mapName", "lost");
	...
	//m_confString[ADVERTISING_BLOCK] = getGlobalString("advertisingBlock", ".net;servegame;no-ip,.net;.com;.com.br;.org;.pl;.net;.biz");
